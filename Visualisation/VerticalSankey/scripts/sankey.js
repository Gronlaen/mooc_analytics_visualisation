d3.sankey = function() {
  ...
  function computeLinkDepths() {
    ...
    nodes.forEach(function(node) {
      ...
      // sum of all incoming flows
      var sum_ty = 0, sum_sy = 0, num_incoming_flows = 0, num_outgoing_flows = 0;
      ...
      var diff_source = (nodeSize - sum_sy)/(num_outgoing_flows-1);
      var sy = 0, ty = 0;
          //ty = node.dy;
      node.sourceLinks.forEach(function(link) {
        link.sy = Math.max(0,Math.min(sy,nodeSize-link.dy));
        sy += link.dy + diff_source;
      });

      if (num_incoming_flows > 1) {
        var diff_target = (nodeSize - sum_ty)/(num_incoming_flows-1);
        ...
        node.targetLinks.forEach(function(link) {
          ...
          link.ty = Math.max(0,Math.min(ty,nodeSize-link.dy));
          ty += link.dy + diff_target;
          //ty -= link.dy;
        });
      } else {
        node.targetLinks.forEach(function(link) {
          link.ty = nodeSize/2 - link.dy/2;
        });
      }
